...
function renderRoute(city: 'bengaluru' | 'varanasi') {
  render(
    <MemoryRouter initialEntries={[`/${city}?window=today`]}>
      <Routes>
...
describe('ExploreRoute', () => {
  ...
  it('does not query or persist a city absent from the enabled-cities response', () => {
    ...
    expect(useEvents).toHaveBeenCalledWith(
      { city: 'varanasi', window: 'today', categories: [], explicitlyFree: false },
      false,
    );
  ...
  it('does not claim freshness from zero calendars', () => {
    vi.mocked(useEvents).mockReturnValue({
      data: {
        pages: [
          {
